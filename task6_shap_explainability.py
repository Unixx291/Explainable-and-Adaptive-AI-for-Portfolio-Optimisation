...
CONFIG = {
    ...
    # Outputs
    "task6_outdir": "results_task6_shap_explainability",

    # Split scope
    "selected_splits": [],          # e.g. ["split_034_2023-07-05_2025-10-06"]
...
def _select_splits(meta_splits: List[Dict], selected_names: List[str], recent_splits: int) -> List[Dict]:
    if selected_names:
...
def main() -> None:
    ...
    print(f"Selected {len(selected_splits)} split(s) for Task 6 SHAP explainability.")

    for split in selected_splits:
        split_name = split["name"]
        print(f"\nRunning Task 6 on split: {split_name}")
        ...
        for rank, pos in enumerate(case_positions, start=1):
            ...
            _save_local_bar_plot(
                ...
                title=f"Task 6 local SHAP drivers: {split_name} case {rank}",
            )
    ...
    summary_df.to_csv(task6_outdir / "task6_shap_summary.csv", index=False)
    local_cases_df.to_csv(task6_outdir / "task6_local_case_summaries.csv", index=False)
    global_texts_df.to_csv(task6_outdir / "task6_global_plain_english.csv", index=False)

    print("\nDone.")
    print(f"Saved SHAP summary:   {task6_outdir / 'task6_shap_summary.csv'}")
    print(f"Saved local summaries:{task6_outdir / 'task6_local_case_summaries.csv'}")
